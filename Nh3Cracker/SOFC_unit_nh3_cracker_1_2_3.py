...
class SofcDesignData(UnitModelBlockData):
    ...
    def _add_constraints(self):
        ...
        @self.Constraint(time)
        def sofc_energy_balance_reactor(b, t):
            Q_s = pyo.units.convert(b.reactor.control_volume.heat[t], to_units=pyo.units.W)
            Q_c = pyo.units.convert(b.cracker.control_volume.heat[t], to_units=pyo.units.W)
            heater_c = pyo.units.convert(b.cracker.control_volume.heat_of_reaction[t], to_units=pyo.units.W)   
            heater_s = pyo.units.convert(b.reactor.control_volume.heat_of_reaction[t], to_units=pyo.units.W) 

            return (heater_c + heater_s == - b.sofc_power_dc[t] - Q_c + Q_s)
    ...
    def _translator_n2_constraints(self):
        ...
        comp_out = set(self.translator_n2.properties_in[t0].mole_frac_comp.keys())
        comp_out.remove("NH3")
        comp_out.remove("N2")
        
        @self.translator_n2.Constraint(time, comp_out)
        def mole_frac_eqn_n2_1(b, t, j): 
            return b.properties_in[t].mole_frac_comp[j] == 1e-19
