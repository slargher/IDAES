...
from idaes.core import MaterialBalanceType, MomentumBalanceType, EnergyBalanceType
...
class SofcDesignData(UnitModelBlockData):
    ...
    def _add_unit_models(self):
        ...
        # 2) NH3 separator into H2 and N2
        ...
        # 3) Translator for N2 stream add prop package
        ...
        # 10) Heater for recycle side
        self.heater_n2 = um.Heater(
            property_package             = self.config.recycle_prop_package,
            has_pressure_change          = False,
        )

        # 11) Translator: water-rich stream → H2 property basis
        ...
        # 12) Mixer for recycled O2 and N2 mix
        ...
        # 13) Heater for cathode side
        self.heater_air = um.Heater(
            property_package             = self.config.cathode_side_prop_package,
            has_pressure_change          = False,
        )

        # 14) Translator: O2-rich stream → air property basis
    ...
    def _add_arcs(self):
        ...
        self.heat_n2_to_mix_out             = Arc(source=self.heater_n2.outlet,
                                                  destination=self.mixer_out.n2_strm)
        ...
        self.heat_o2_to_mix_air               = Arc(source=self.heater_air.outlet,
                                                destination=self.mixer_air.o2_poor_strm)
    ...
    def _add_constraints(self):
        ...
        @self.Constraint(time)
        def sofc_energy_balance_1(b, t):
            return (pyo.units.convert(b.heater_air.control_volume.heat[t], to_units=pyo.units.W) 
                   == pyo.units.convert(b.reactor.control_volume.heat_of_reaction[t], to_units=pyo.units.W)+ pyo.units.convert(b.reactor.control_volume.heat[t], to_units=pyo.units.W))


        @self.Constraint(time)
        def sofc_energy_balance_2(b, t):
            return (pyo.units.convert(b.heater_n2.control_volume.heat[t], to_units=pyo.units.W) 
                   == pyo.units.convert(b.cracker.control_volume.heat_of_reaction[t], to_units=pyo.units.W)+ pyo.units.convert(b.cracker.control_volume.heat[t], to_units=pyo.units.W))
    ...
    def _translator_n2_constraints(self):
        ...
        @self.translator_n2.Constraint(time)
        def mole_frac_eqn_n2_0(b, t): 
            return b.properties_out[t].mole_frac_comp['NH3'] == b.properties_in[t].mole_frac_comp['NH3']
        
        #adding this new
        @self.translator_n2.Constraint(time)
        def mole_frac_eqn_n2_1(b, t):
            return b.properties_out[t].mole_frac_comp['N2'] == b.properties_in[t].mole_frac_comp['N2']

        comp_out = set(self.translator_n2.properties_out[t0].mole_frac_comp.keys())
        comp_out.remove("NH3")
        comp_out.remove("N2")
        comp_out.remove("H2")
        
        @self.translator_n2.Constraint(time, comp_out)
        def mole_frac_eqn_n2_2(b, t, j): 
            return b.properties_out[t].mole_frac_comp[j] == 1e-19
    ...
    def _translator_h2_constraints(self):
        ...
        @self.translator_h2.Constraint(time)
        def mole_frac_eqn_h2(b, t): 
            return b.properties_out[t].mole_frac_comp['H2'] == b.properties_in[t].mole_frac_comp['H2']

        comps_out = set(self.translator_h2.properties_out[t0].mole_frac_comp.keys())
        comps_out.remove("H2")
        comps_out.remove("O2")

        @self.translator_h2.Constraint(time, comps_out)
